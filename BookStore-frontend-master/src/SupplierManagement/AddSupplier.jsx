  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const isDuplicate = suppliers.some(
        (supplier) => supplier.supName === values.supplierName
      );
        ...
        >
            ...
            rules={[
              {
                validator: (_, value) => {
                  ...
                  if (!/^[A-Za-z0-9\s]+$/.test(value)) {
                    return Promise.reject(
                      new Error(
                        "Supplier name cannot contain special characters"
                      )
                    );
                  }
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Supplier email cannot be empty or only spaces",
              },
            ...
            rules={[
              {
                validator: (_, value) => {
                  if (!value || value.trim() === "") {
                    return Promise.reject(
                      new Error(
                        "Supplier address cannot be empty or only spaces"
                      )
                    );
                  }
                  return Promise.resolve();
