    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          message.error(
            `Failed to delete promotion: ${error.response?.data || "Unknown error"
            }`
          );
        }
      },
              ...
              <Timeline>
                {paginatedLogs.map((log) => (
                  ...
                  >
                    <p>
                      {log.proAction === 1
                        ? `${log.username || "Unknown"} created promotion "${log.proName || "Unknown Promotion"
                        }"`
                        : log.proAction === 2
                          ? `${log.username || "Unknown"} approved promotion "${log.proName || "Unknown Promotion"
                          }"`
                          : log.proAction === 3
                            ? `${log.username || "Unknown"} rejected promotion "${log.proName || "Unknown Promotion"
                            }"`
                            : log.proAction === 4
                              ? `${log.username || "Unknown"} deleted promotion "${log.proName || "Unknown Promotion"
                              }"`
                              : ""}
                    </p>
                    <small>
